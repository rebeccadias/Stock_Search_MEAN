...
export class SearchBarComponent {
  ...
  options: string[] = [];
  // home.component.ts
  filteredOptions: Observable<{symbol: string, description: string}[]> | undefined;
  ...
  private _filter(value: any): Observable<any[]> {
    ...
    return this.appService.searchStocks(searchString.trim()).pipe(
  ...
  performSearch() {
    ...
    if (typeof tickerValue === 'object' && tickerValue !== null && tickerValue.symbol) {
...
displayFn(stock: any): string {
  return stock && stock.symbol ? `${stock.symbol} | ${stock.description}` : '';
}
